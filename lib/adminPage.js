...
// Поля: клиент, стоимость работ, динамические доп. расходы (название+цена+период),
// живой подсчёт суммы, блок результата со ссылкой.
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  h2.sec{font-family:'Space Grotesk';font-size:14px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin-bottom:6px}
  .hint{font-size:12px;color:var(--muted);margin-bottom:14px}
  .price{font-family:'JetBrains Mono';font-weight:700;text-align:right}
  .exp{display:grid;grid-template-columns:1fr 130px 150px 44px;gap:10px;margin-bottom:10px;align-items:center}
  ...
  @media(max-width:580px){.row2{grid-template-columns:1fr}.exp{grid-template-columns:1fr 100px 44px}.exp .period{grid-column:1 / span 2}}
</style>
</head>
<body>
  <div class="hero">
    ...
    <div class="card" id="formCard">
      ...
      <div class="row2">
        <div class="field">
          ...
          <input type="text" id="clientName" placeholder="Мысалы: Zere Clinic">
        </div>
        <div class="field">
      ...
      <div class="field">
        <label>Жұмыс құны (енгізу және оқыту), ₸</label>
        <input type="number" id="workCost" class="price" value="250000" min="0">
      </div>

      <h2 class="sec">Қосымша шығындар</h2>
      <div class="hint">Әр жол: атауы + бағасы (₸) + төлем кезеңі</div>
      <div id="expenses"></div>
      <button class="addbtn" id="addBtn" type="button">+ Жол қосу</button>

      <div class="totbar">
        <div class="lbl">Жалпы сома (жұмыс + шығындар)</div>
...
<script>
  ...
  var workEl = document.getElementById('workCost');

  function addRow(name, price, period) {
    var row = document.createElement('div');
    row.className = 'exp';
    row.innerHTML =
      '<input type="text" class="ename" placeholder="Атауы">' +
      '<input type="number" class="price eprice" placeholder="Бағасы ₸" min="0">' +
      '<input type="text" class="period" placeholder="Төлем кезеңі">' +
      '<button type="button" class="del" title="Жою">×</button>';
    expensesEl.appendChild(row);
    if (name) row.querySelector('.ename').value = name;
    if (price) row.querySelector('.eprice').value = price;
    if (period) row.querySelector('.period').value = period;
    row.querySelector('.del').addEventListener('click', function () { row.remove(); recalc(); });
    row.querySelector('.eprice').addEventListener('input', recalc);
  }

  function recalc() {
    var total = parseFloat(workEl.value) || 0;
    document.querySelectorAll('.eprice').forEach(function (i) { total += parseFloat(i.value) || 0; });
  ...
  function collectExpenses() {
    ...
    document.querySelectorAll('.exp').forEach(function (row) {
      var name = row.querySelector('.ename').value.trim();
      var price = parseFloat(row.querySelector('.eprice').value) || 0;
      var period = row.querySelector('.period').value.trim();
      if (name && price > 0) list.push({ name: name, price: price, period: period });
    });
    return list;
  }

  document.getElementById('addBtn').addEventListener('click', function () { addRow(); });
  workEl.addEventListener('input', recalc);

  // Стартовые строки-примеры (можно изменить или удалить)
  addRow('Лицензии amoCRM (2 аккаунта)', 96000, 'на 6 месяцев');
  addRow('Сервис WAZZUP (1 канал)', 194400, 'каждые 6 месяцев');
  addRow('Sipuni (1 аккаунт)', 24600, 'каждые 6 месяцев');
  recalc();
  ...
  createBtn.addEventListener('click', async function () {
    ...
    var workCost = parseFloat(workEl.value) || 0;
    ...
    if (workCost <= 0) { errMsg.textContent = 'Жұмыс құнын енгізіңіз.'; return; }
    ...
    try {
      var res = await fetch('/api/create', {
        ...
        body: JSON.stringify({ clientName: clientName, company: company, workCost: workCost, expenses: expenses, validDays: validDays, showRub: showRub, adminPassword: adminPassword })
      });
